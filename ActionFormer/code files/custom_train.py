...
def filter_checkpoint_for_model(checkpoint_state_dict, model_state_dict):
    """
    Filter out incompatible layers from checkpoint state dict
    """
    filtered_dict = {}
...
def main():
    ...
    if args.resume != None:
        logger.info("Loading pre-trained weights from: {}".format(args.resume))
        device = f"cuda:{args.local_rank}"
        checkpoint = torch.load(args.resume, map_location=device)
        
        # Get the source state dict based on what's available
        if "state_dict_ema" in checkpoint:
            source_state_dict = checkpoint["state_dict_ema"]
        ...
        # Get the target model's state dict
        model_state_dict = model.state_dict()
        
        # Filter out incompatible parameters
        filtered_state_dict = filter_checkpoint_for_model(source_state_dict, model_state_dict)
        ...
        if model_ema is not None and "state_dict_ema" in checkpoint:
            # For EMA model, also filter out incompatible parameters
            ema_state_dict = model_ema.module.state_dict()
            filtered_ema_dict = filter_checkpoint_for_model(checkpoint["state_dict_ema"], ema_state_dict)
            model_ema.module.load_state_dict(filtered_ema_dict, strict=False)
        
        del checkpoint  # Save memory
        torch.cuda.empty_cache()
        
        # Start from epoch 0 for fine-tuning
        resume_epoch = -1
    else:
        resume_epoch = -1
